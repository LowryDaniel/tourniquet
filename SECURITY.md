...
Email **security@tourniquet.ai** with:
...
- Tourniquet token forgery or bypass
- SQL injection
- SSRF via the proxy
...
- **Tourniquet tokens** (`tq_*`) are stored as bcrypt hashes. The plaintext token is shown exactly once on creation and never stored.
